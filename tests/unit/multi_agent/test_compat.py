...
from agentos.execution.ports import Accepted, CreateExecution, Indeterminate
...
def test_execution_creation_preserves_indeterminate_commit_state():
    class Control:
        def create(self, command):
            return Indeterminate(transaction_id="tx:unknown")
    ...
    request = DelegateTask(
        ...
        handoff_ref=SimpleNamespace(handoff_id="handoff:1", version=1),
        child_limits=SimpleNamespace(max_duration_seconds=60, max_iterations=5, max_cost=None, max_provider_tokens=None),
        deadline_at=None, purpose="task.delegation", classification="INTERNAL", authorization_ref="auth:1",
        failure_policy=DelegationFailurePolicy.PROPAGATE,
        cancellation_policy=DelegationCancellationPolicy.CANCEL_CHILD_ONLY,
        correlation_id="corr:1", causation_id="command:1", idempotency_key="delegation:unknown", requested_at=NOW,
    )
    receipt = adapter.create_child(request=request, execution_id="execution:unknown", resolved_agent=SimpleNamespace(config_version=1))
