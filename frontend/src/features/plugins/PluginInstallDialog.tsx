...
export function PluginInstallDialog({ client, onClose, onInstalled, initialReference, onNoManifest }: { client: ApiClient; onClose: () => void; onInstalled: () => void; initialReference?: string; onNoManifest?: () => void }) {
  ...
  // The auto-inspection runs as a subscription: every state change happens in
  // a promise callback, never in the effect body. The dialog already opens
  // busy when it was handed a reference, so nothing has to be set up front.
  useEffect(() => {
    if (!initialReference) return
    let active = true
    inspectPlugin(client, initialReference)
      .then((result) => { if (active) setInspection(result) })
      .catch((caught) => { if (active) applyFailure(caught) })
      .finally(() => { if (active) setBusy(false) })
    return () => { active = false }
  }, [initialReference, client, applyFailure])
