...
export function ContextIndicator({ usage }: { usage: ContextUsage | null }) {
  ...
  const tone = usage.percentage >= 90 ? 'is-danger' : usage.percentage >= 75 ? 'is-warn' : ''
  const rows = [
  ...
  return (
    <div className={`context-indicator ${tone}`} tabIndex={0} aria-label={`Contexto: ${usage.used_tokens.toLocaleString('pt-BR')} de ${usage.limit_tokens.toLocaleString('pt-BR')} tokens`}>
      <span className="context-indicator__dot" aria-hidden="true" />
      <span>Contexto {usage.used_tokens.toLocaleString('pt-BR')} / {usage.limit_tokens.toLocaleString('pt-BR')}</span>
