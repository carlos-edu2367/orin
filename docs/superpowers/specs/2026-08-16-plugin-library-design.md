...
- **Web search**: reuses the pattern established by `agentic/web_search.py` (`BraveSearchClient`, `search_client_from_environment()`). The service issues a small, fixed set of queries targeting MCP plugin repositories (e.g. `"mcp server" plugin.json github`), and keeps only results whose URL passes the same public-GitHub allow-list `sources.py::resolve_source` already enforces for manual installs. If `search_client_from_environment()` returns `None` (no `AGENTOS_SEARCH_API_KEY` configured), the web source is skipped entirely — the service still returns registry results, and the response is marked so the frontend can show a quiet "web search unavailable" note instead of an error.
