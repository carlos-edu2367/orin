...
- [x] Step 1: Write tests for frozen models, complete contexts, exact versions/status, positive limits, bounded structured values, no secret-like checkpoint fields, and canonical state mapping.
- [x] Step 2: Run `python -m pytest -q tests/unit/capabilities/test_contracts.py`; confirmed failure because the package was absent.
- [x] Step 3: Implement only the immutable bounded models, validation and `execution_state_for_capability_state`.
- [x] Step 4: Re-run the focused test and then `python -m pytest -q tests/unit/capabilities/test_contracts.py` until green.
...
- [x] Step 1: Write tests for immutable version conflict, authorized resolve/list, disable expected status, bootstrap only when empty with `SYSTEM_BOOTSTRAP`, and no authorization by known ID.
- [x] Step 2: Run the focused registry tests and confirmed the expected missing-package failure.
- [x] Step 3: Implement the registry with copied immutable descriptors/programs, scoped idempotency fingerprints, status filtering and minimal audit records.
- [x] Step 4: Run focused registry tests green and check reprs contain no program payload.
...
- [x] Step 1: Write tests for explicit dependencies, cycle/duplicate/binding rejection, deterministic ordering, maximum parallelism, no implicit fan-out, and structural boundary scans.
- [x] Step 2: Run focused tests and verified RED.
- [x] Step 3: Implement pure scheduler, protocols and `InMemoryCapabilityState` with allowlisted snapshots and outbox records.
- [x] Step 4: Run focused tests green and inspect imports for concrete technology/bypass terms.
...
- [x] Step 1: Write tests for queued start, duplicate start, expected state-version conflict, acquire/elegibility, tool invocation through port, waiting mappings, child pause/resume, terminal immutability, inspect authorization and cancellation propagation.
- [x] Step 2: Run focused lifecycle tests and verified RED.
- [x] Step 3: Implement minimal service command flow, committing state before Execution transition when required, and map only through `ExecutionControl`.
- [x] Step 4: Run lifecycle tests green and add regression tests for stale writer/late result/terminal retry.
...
- [x] Step 1: Write tests for denied Tool despite broad Capability grants, no escalation from untrusted arguments, all maximum limits, safe retry only, UNKNOWN requiring reconcile, ordered compensation/failure visibility and cancellation budget.
- [x] Step 2: Run focused tests and verified RED.
- [x] Step 3: Implement authorization intersection, limit accounting, retry classifier, reconcile path and compensation runner using only ports.
- [x] Step 4: Run focused tests green and run the package boundary scan.
...
- [x] Step 1: Write end-to-end boundary tests proving Capability -> ExecutionControl -> Tool port/child port -> state/outbox, plus optional PostgreSQL skip test when DSN is absent.
- [x] Step 2: Run the new integration tests RED where dependencies are absent, recording explicit skips only for optional DSN/runtime.
- [x] Step 3: Implement only test fixtures/verification helpers needed to prove the existing ports; do not add production adapters.
- [x] Step 4: Run focused integration tests green.
- [x] Step 5: Execute `python -m pytest -q`, `python -m compileall -q src tests`, `git diff --check`, `git status --short --branch` and the required `rg` scans.
- [x] Step 6: Fill the matrix and closeout with actual output, independent read-only review findings, commits, skips and the documented next gate; scan for TODO/placeholders and preserve unrelated worktree changes.
