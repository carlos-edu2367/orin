...
def test_rrf_rewards_a_document_ranked_well_by_both_rankings() -> None:
    scores = reciprocal_rank_fusion([["a", "b", "c"], ["c", "b", "a"]])
...
def test_a_mutating_tool_queues_the_changed_paths_for_reindexing(tmp_path) -> None:
    retrieval = _FakeRetrieval()
    tools = AgentToolset(ConversationWorkspace(tmp_path, "chat_rag_write"), retrieval=retrieval)

    tools.invoke("write_file", {"path": "notes/plan.md", "content": "step one"})

    assert retrieval.reindexed == [["notes/plan.md"]]
```
...
In `_toolset` (line 813), add the argument after `search_client=self.search_client,`:
