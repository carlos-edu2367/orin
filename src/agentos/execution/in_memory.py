...
class InMemoryTransactionalPersistence(TransactionalPersistence):
    ...
    def transact(self, request: TransactionRequest) -> TransactionResult:
        ...
        if request.change.new_execution.execution_id != request.context.execution_id:
            return TransactionRejected(RejectionReason.UNAUTHORIZED)

        receipt = TransactionReceipt(
            transaction_id=request.transaction_id,
            commit_state=TransactionCommitState.COMMITTED,
            affected_execution_id=request.change.new_execution.execution_id,
            resulting_version=request.change.new_execution.state_version,
            outbox_event_ids=(request.outbox.event.event_id,),
        )

        # These three writes are one in-memory commit unit: no mutation occurs before all checks above pass.
        self._executions[str(request.change.new_execution.execution_id)] = request.change.new_execution
        ...
        self._idempotency[idempotency_key] = _StoredIdempotency(
            ...
            receipt=receipt,
            execution=request.change.new_execution,
        )

        if self._indeterminate:
            self._indeterminate = False
            return TransactionIndeterminate(request.transaction_id)
        return TransactionCommitted(receipt, request.change.new_execution)
    ...
    @staticmethod
    def _key(context: ExecutionCommandContext, idempotency_key: IdempotencyKey) -> tuple[str, str, str]:
        return str(context.user_id), str(context.purpose), str(idempotency_key)
